        ...
        <form wire:submit="save" class="p-6 space-y-6">
            ...
            <div>
                <label for="banner" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
                    Ảnh Banner (tùy chọn) <span class="text-xs font-normal text-gray-500">- Chỉ chọn 1 ảnh</span>
                </label>
                
                @if ($banner)
                    {{-- Hiển thị banner đã chọn --}}
                    <div class="relative inline-block">
                        <img src="{{ $banner->temporaryUrl() }}" alt="Preview Banner" class="h-40 w-full max-w-md rounded-xl object-cover shadow-lg border-2 border-emerald-500" />
                        <button 
                            type="button" 
                        ...
                        <div class="mt-2 text-xs text-emerald-600 dark:text-emerald-400 font-medium">
                            ✓ Banner đã chọn - Click X để thay đổi
                        </div>
                    </div>
                @else
                    {{-- Nút chọn banner --}}
                    <label class="flex cursor-pointer items-center justify-center gap-3 rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 px-6 py-8 hover:border-emerald-500 hover:bg-emerald-50 dark:border-gray-600 dark:bg-gray-700 dark:hover:border-emerald-500 dark:hover:bg-emerald-900/20 transition-all max-w-md">
